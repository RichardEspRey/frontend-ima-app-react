//Componente para editar cabeceras de el trip sin restriccion de datos 
...
import useFetchActiveDrivers from '../hooks/Edit_Trips_complete/useFetchActiveDrivers';
import useFetchActiveTrucks from '../hooks/Edit_Trips_complete/useFetchActiveTrucks';
import useFetchActiveTrailers from '../hooks/Edit_Trips_complete/useFetchActiveTrailers';
...
const EditTripComplete = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        is_transnational: '0', transnational_number: '', movement_number: ''
    });
    const [etapas, setEtapas] = useState([]);
    const [trailerType, setTrailerType] = useState('interna');
    const [transnationalTrips, setTransnationalTrips] = useState([]);
    const [isContinuation, setIsContinuation] = useState(false);
    const [isExistingTransnationalTrip, setIsExistingTransnationalTrip] = useState(false);
        ...
        const fetchTripDetails = async () => {
            ...
            try {
                ...
                if (res.ok && result.status === "success" && result.trip) {
                    ...
                    setFormData({
                        trip_number: trip.trip_number || '', driver_id: trip.driver_id || '', driver_id_second: trip.driver_id_second || '',
                        driver_nombre: trip.driver_nombre || '', driver_second_nombre: trip.driver_second_nombre || '', truck_id: trip.truck_id || '',
                        truck_unidad: trip.truck_unidad || '', caja_id: trip.caja_id || '', caja_no_caja: trip.caja_no_caja || '',
                        caja_externa_id: trip.caja_externa_id || '', caja_externa_no_caja: trip.caja_externa_no_caja || '',
                        return_date: trip.return_date ? parseISO(trip.return_date) : null, status: trip.status || 'In Transit', country_code: trip.country_code || '',
                        is_transnational: trip.is_transnational || '0', transnational_number: trip.transnational_number || '', movement_number: trip.movement_number || ''
                    });
                    setIsContinuation(!!trip.transnational_number);
                    setIsExistingTransnationalTrip(!!trip.transnational_number);
                    ...
                    const processedEtapas = result.etapas.map(etapa => {
                        ...
                        const stops = Array.isArray(etapa.stops_in_transit) ? etapa.stops_in_transit.map(stop => ({
                            ...stop, bl_firmado_doc: stop.bl_firmado_doc ? {
                                fileName: stop.bl_firmado_doc.nombre_archivo?.split(/[\\/]/).pop() || 'Archivo',
                                vencimiento: stop.bl_firmado_doc.fecha_vencimiento || null, file: null, document_id: stop.bl_firmado_doc.document_id, serverPath: stop.bl_firmado_doc.path_servidor_real
                            } : null
                        })) : [];
                        ...
                        return {
                            ...
                            date_of_departure: etapa.date_of_departure ? parseISO(etapa.date_of_departure) : null, // 🚨 PERSISTENCIA NUEVA FECHA
                            documentos: baseDocs, stops_in_transit: stops, comments: etapa.comments || '', time_of_delivery: etapa.time_of_delivery || ''
                        };
                    });
    ...
    // --- VIAJES TRANSNACIONALES (para vincular, según país base seleccionado) ---
    useEffect(() => {
        const tripYear = initialTripData?.trip?.trip_year;
        const oppositeCountry = formData.country_code === 'MX' ? 'US' : formData.country_code === 'US' ? 'MX' : '';

        if (formData.is_transnational !== '1' || !oppositeCountry || !tripYear || isExistingTransnationalTrip) {
        ...
        fd.append('country_code', oppositeCountry);
        fd.append('trip_year', tripYear);

        fetch(`${apiHost}/new_tripsv2.php`, { method: 'POST', body: fd })
            .then((res) => res.json())
            .then((data) => setTransnationalTrips(data.status === 'success' ? data.data || [] : []))
            .catch(() => setTransnationalTrips([]));
    }, [apiHost, formData.is_transnational, formData.country_code, initialTripData, isExistingTransnationalTrip]);

    // --- HANDLERS ---
    const handleFormChange = (name, value) => setFormData(p => ({ ...p, [name]: value }));
    const handleTransnationalChange = (checked) => {
        handleFormChange('is_transnational', checked ? '1' : '0');
        if (!checked) {
            setIsContinuation(false);
            setIsExistingTransnationalTrip(false);
            handleFormChange('transnational_number', '');
            handleFormChange('movement_number', '');
        }
    };
    const handleContinuationChange = (checked) => {
        setIsContinuation(checked);
        if (!checked) handleFormChange('transnational_number', '');
    };
    ...
    const handleCreateEntity = async (inputValue, stageIdx, fieldKey, endpoint, op, refetch) => {
        ...
        try {
            ...
            if (result.status === "success") {
                ...
                refetch();
                if (stageIdx === null) handleFormChange(fieldKey, id); else handleStageChange(stageIdx, fieldKey, id);
                Swal.fire('Éxito', `Creado: ${label}`, 'success'); return { value: id, label: label };
            } else { throw new Error(result.message); }
        } catch (e) { Swal.fire('Error', e.message, 'error'); } finally { setter(false); }
    };
    ...
    const handleGuardarDocumento = (data) => {
        ...
        setEtapas(prev => {
            ...
            if (stopIndex !== null) {
                ...
                stops[stopIndex][docType] = newDoc; copy[stageIndex].stops_in_transit = stops;
            } else {
    ...
    const eliminarParadaEnRuta = (stageIdx, stopIdx) => {
        Swal.fire({ title: '¿Eliminar Parada?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Sí', cancelButtonText: 'No' }).then(r => {
            if (r.isConfirmed) setEtapas(prev => { const copy = [...prev]; copy[stageIdx].stops_in_transit = copy[stageIdx].stops_in_transit.filter((_, i) => i !== stopIdx); return copy; });
        });
    };
    ...
    const agregarNuevaEtapa = (type) => {
        ...
        const newStage = {
            trip_stage_id: `new-stage-${Date.now()}`, stageType: type, invoice_number: '', origin: '', destination: '', estatus: type === 'borderCrossing' ? 'In Coming' : 'In Transit',
            documentos: baseDocs, stops_in_transit: [], comments: '', loading_date: null, delivery_date: null, date_of_departure: null
        };
    ...
    const handleSaveExternalCaja = async (cajaData) => {
        const fd = new FormData(); fd.append('op', 'Alta'); Object.entries(cajaData).forEach(([k, v]) => fd.append(k, v));
        try {
            const res = await fetch(`${apiHost}/caja_externa.php`, { method: 'POST', body: fd });
            const r = await res.json();
            if (r.status === 'success') {
                handleFormChange('caja_externa_id', r.caja.caja_externa_id); handleFormChange('caja_externa_no_caja', r.caja.no_caja);
                handleFormChange('caja_id', ''); refetchExternalTrailers(); setIsModalCajaExternaOpen(false); Swal.fire('Éxito', 'Caja creada', 'success');
            } else { throw new Error(r.message); }
        } catch (e) { Swal.fire('Error', e.message, 'error'); }
    };

    const options = {
        drivers: activeDrivers.map(d => ({ value: d.driver_id, label: d.nombre })), trucks: activeTrucks.map(t => ({ value: t.truck_id, label: t.unidad })),
        trailers: activeTrailers.map(c => ({ value: c.caja_id, label: c.no_caja })), externalTrailers: activeExternalTrailers.map(c => ({ value: c.caja_externa_id, label: c.no_caja })),
    ...
    return (
        <Container maxWidth="xl" sx={{ py: 3, pb: 10 }}>
            ...
            {/* General Info */}
            <GeneralTripInfoComplete
                formData={formData} handleFormChange={handleFormChange} tripMode={tripMode} handleTripModeChange={handleTripModeChange} trailerType={trailerType}
                handleTrailerTypeChange={handleTrailerTypeChange} isFormDisabled={formData.status === 'Completed'} options={options}
                ...
                transnationalTrips={transnationalTrips}
                isContinuation={isContinuation}
                isExistingTransnationalTrip={isExistingTransnationalTrip}
                onTransnationalChange={handleTransnationalChange}
                onContinuationChange={handleContinuationChange}
            />

            {/* Listado de Etapas Limpio */}
            <StageList
                etapas={etapas}
                handleStageChange={handleStageChange} eliminarEtapa={eliminarEtapa} agregarParadaEnRuta={agregarParadaEnRuta} eliminarParadaEnRuta={eliminarParadaEnRuta}
                handleStopChange={handleStopChange} abrirModal={abrirModal} isFormDisabled={formData.status === 'Completed'} options={options}
                creators={{
...
export default EditTripComplete;