...
const ProfileAccessManager = () => {
    ...
    const fetchUsers = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(`${apiHost}/features.php`, {
                method: 'POST',
                body: formData,
            });
    ...
    const handlePermissionChange = async (userId, sectionName, canView) => {
        ...
        const updatedUsers = users.map(u => {
            if (u.id === userId) {
                userUpdated = true;
                return { ...u, permissions: { ...u.permissions, [sectionName]: canView } };
            }
            return u;
        });
        ...
        if (userUpdated && selectedUser && selectedUser.id === userId) {
            ...
            if (updatedUserInModal) setSelectedUser(updatedUserInModal);
        }
    ...
    const handleToggleFeature = async (userId, featureId, plataform, enabled) => {
        const updater = (features) =>
            features.map(f => f.feature_id === featureId ? { ...f, enabled: enabled ? 1 : 0 } : f);
    ...
    const handleCreateUser = async () => {
        ...
        try {
            const formData = new FormData();
            formData.append('op', 'create_user');
            ...
            <Dialog open={newUserModal.open} onClose={handleCloseNewUser} maxWidth="sm" fullWidth>
                ...
                <DialogContent>
                    <Stack spacing={2} sx={{ mt: 1 }}>
                        ...
                        <TextField
                            label="Usuario"
                            value={newUserModal.user}
                            onChange={handleNewUserChange('user')}
                            fullWidth
                            size="small"
                        />
                        <TextField
                            label="Contraseña"
                            type="password"
                            value={newUserModal.pass}
                            onChange={handleNewUserChange('pass')}
                            fullWidth
                            size="small"
                        />
